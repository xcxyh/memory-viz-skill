...
# 设置路径
WWW_DIR="$SKILL_DIR/assets/memory-viz-www"
PORT=3001
...
# 检查进程
if pgrep -f "node.*memory-viz.*server.js" > /dev/null; then
    PID=$(pgrep -f "node.*memory-viz.*server.js" | head -1)
    echo "✅ 状态: 运行中"
    echo "🔢 PID: $PID"
    ...
    echo "📁 项目目录: $WWW_DIR"
    echo ""
    echo "提示：使用 'bash $SCRIPT_DIR/start.sh' 启动服务"
fi
